if(os_type==os_ios){
    ...
    if (type == "camera") {
        ...
        if (event == "picked") {
            var filename = async_load[? "filename"];
            var processed_filename = camera_process_picture(filename, CAMERA_FORMAT_PNG, 2048, 2048);
            if(image_loaded){
                sprite_delete(image);
                image_loaded=false;
            }
            /*if(global.show_palettes){
                global.show_palettes=false;
                view_visible[2]=false;
                view_visible[3]=false;
                view_visible[1]=true;
            }*/
            //filter_alpha=.3;
            //filter_subdivision=4;
            //global.c_blend=0;
            image = sprite_add(processed_filename, 0, false, false, 0, 0);
            alarm[2]=5;
        }
        if (event == "dismissed") {
...
//show_debug_message("Right after picture got");
